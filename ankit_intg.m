...
if iex ==1
    ...
    area1= h/2*(y_sum(1)+y_sum(Nsamples-1)+ sum(b))
...
if iex ==2
    ...
    % Discretize X for summation
    xmin = 0; xmax = 4;
    Nsamples = 15; % Number of points for dividing x (N-1 rectangles)
    x_sum = linspace(xmin,xmax,Nsamples);
    h = x_sum(2) - x_sum(1);
...
if iex == 3
    ...
    area1 =h/3 (y_sum(1)+y_sum(7)+2*y_sum(2)+2*y_sum(4)+2*y_sum(6)+4*y_sum(3)+4*y_sum(5));
